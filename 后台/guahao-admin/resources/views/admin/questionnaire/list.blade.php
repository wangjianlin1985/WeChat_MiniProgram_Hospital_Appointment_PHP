    ...
    <script>
        layui.use(["element", "form", "table", "layer"], function () {
            ...
            form.on('submit(search)', function (data) {
                table.reload("layuiTable", { url: "/admin/questionnaire/getTable", method: "get", where: data.field });
                return false;
            });
...
@section('content')
    <div class="bar"><a href="">首页</a><a>问卷调查管理</a><span>问卷调查列表</span></div>
    <div class="grid">
        ...
        <div class="gridTable">
            <div class="layui-tables">
                <table id="layuiTable" class="layui-table" lay-data="{url:'/admin/questionnaire/getTable',method:'get',page:true}" lay-filter="opt">
                    <thead>
                    <tr>
                        <th lay-data="{field:'num',type:'numbers'}"></th>
                        <th lay-data="{field:'title'}">标题</th>
                        <th lay-data="{field:'counts',width:120}">参与人数</th>
                        <th lay-data="{field:'created_at',width:160}">添加时间</th>
                        <th lay-data="{field:'updated_at',width:160}">更新时间</th>
                        <th lay-data="{field:'opt',width:130,toolbar:'#optbar'}">操作</th>
                    </tr>
                    </thead>
                </table>
            </div>
    ...
    @verbatim
        <script type="text/html" id="optbar">
            <a href="/admin/questionnaire/edit/{{d.id}}">编辑</a>
            <a href="javascript:;" lay-event="del">删除</a>
            <a href="/admin/questionnaire/question/{{d.id}}">题目</a>
        </script>
    @endverbatim
@endsection

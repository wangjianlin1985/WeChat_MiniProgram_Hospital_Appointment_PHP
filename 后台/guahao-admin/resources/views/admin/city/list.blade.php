    ...
    <script>
        layui.use(["element", "form", "table", "layer"], function () {
            ...
            form.on('submit(search)', function (data) {
                table.reload("layuiTable", { url: "/admin/city/getTable", method: "get", where: data.field });
                return false;
            });
...
@section('content')
    <div class="bar"><a href="">首页</a><a>城市信息管理</a><span>城市信息列表</span></div>
    <div class="grid">
            ...
            <div class="flex-row">
                <div class="flex-col-6">
                    <form class="layui-form">
                        <span>标题：</span>
                        <input type="text" class="xx" name="name" autocomplete="off"/>
                        <button class="btn" lay-submit lay-filter="search" type="button">查询</button>
                    </form>
                </div>
                <div class="flex-col-6 txt-right">
                    <a class="btn" href="/admin/city/add">新增</a>
                </div>
            </div>
        </div>
        <div class="gridTable">
            <div class="layui-tables">
                <table id="layuiTable" class="layui-table" lay-data="{url:'/admin/city/getTable',method:'get',page:true}" lay-filter="opt">
                    <thead>
                    <tr>
                        <th lay-data="{field:'num',type:'numbers'}"></th>
                        <th lay-data="{field:'name'}">名称</th>
                        <th lay-data="{field:'hospital',toolbar:'#hospitalTpl'}">医院</th>
    ...
    @verbatim
        <script type="text/html" id="hospitalTpl">
            <a href="/admin/hospital/list?city_id={{d.id}}">{{ d.hospital }}</a>
        </script>
        <script type="text/html" id="optbar">
            <a href="/admin/city/edit/{{d.id}}">编辑</a>
